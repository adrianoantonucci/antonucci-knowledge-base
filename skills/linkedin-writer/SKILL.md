---
name: linkedin-writer
description: Transforma experiências reais, incidentes, projetos e decisões de engenharia em conteúdo para LinkedIn focado em construir autoridade técnica e posicionamento profissional (DevOps, Kubernetes, FinOps, Platform Engineering, Liderança Técnica, IA aplicada à engenharia). Use sempre que o usuário pedir para escrever, redigir, criar ou estruturar um post de LinkedIn, artigo, carrossel, ou transformar um incidente/projeto/aprendizado em conteúdo compartilhável — mesmo que ele não diga explicitamente "LinkedIn".
---

# LinkedIn Writer

## Objetivo

Transformar experiências reais, aprendizados, projetos, incidentes, desafios técnicos e decisões de engenharia em conteúdo relevante para LinkedIn.

O foco principal não é gerar curtidas.

O foco principal é construir autoridade técnica, credibilidade profissional e posicionamento estratégico.

---

## Público-Alvo

Priorizar conteúdo para:

* DevOps Engineers
* Platform Engineers
* SREs
* Cloud Engineers
* Tech Leads
* Engineering Managers
* Arquitetos de Solução
* Staff Engineers

---

## Objetivos do Conteúdo

Sempre buscar um ou mais dos objetivos abaixo:

* Compartilhar conhecimento
* Demonstrar experiência prática
* Mostrar capacidade de resolução de problemas
* Demonstrar pensamento estratégico
* Compartilhar aprendizados
* Fortalecer marca pessoal
* Construir networking
* Gerar oportunidades profissionais

---

## Tom de Comunicação

Utilizar:

* Linguagem simples
* Linguagem profissional
* Experiência prática
* Transparência
* Autenticidade

Evitar:

* Clickbait
* Sensacionalismo
* Arrogância
* Promessas irreais
* Conteúdo genérico

---

## Princípio Fundamental

Priorizar sempre experiências reais.

Melhor compartilhar:

"Como resolvemos um problema de custos no Kubernetes"

do que:

"10 dicas genéricas sobre Kubernetes"

---

## Estrutura Padrão de Post

### 1. Gancho

Abrir com:

* Problema real
* Resultado obtido
* Aprendizado relevante
* Pergunta provocativa

Exemplos:

* Encontramos um deployment consumindo 4x mais CPU do que precisava.
* Um ajuste simples reduziu nosso custo em Kubernetes.
* Nem todo problema de performance é problema de código.
* O maior gargalo nem sempre está na tecnologia.

### 2. Contexto

Explicar rapidamente:

* Cenário
* Problema
* Impacto

### 3. Solução

Descrever:

* O que foi feito
* Como foi feito
* Quais decisões foram tomadas

### 4. Aprendizado

Destacar:

* Lições aprendidas
* Erros evitados
* Boas práticas

### 5. Encerramento

Finalizar com:

* Reflexão
* Pergunta
* Convite para troca de experiências

---

## Framework para Conteúdo Técnico

Ao receber um tema, identificar:

* Problema
* Impacto
* Decisão
* Resultado
* Aprendizado

Gerar conteúdo baseado nesses cinco elementos.

---

## Temas Prioritários

### DevOps

* CI/CD
* GitLab
* Automação
* Observabilidade

### Kubernetes

* OOMKilled
* HPA
* Requests e Limits
* Troubleshooting
* Performance

### FinOps

* Redução de custos
* Rightsizing
* Otimização de clusters
* Eficiência operacional

### Platform Engineering

* Self-Service
* Padronização
* Developer Experience

### Liderança Técnica

* Mentoria
* Gestão de contexto
* Tomada de decisão
* Escalabilidade organizacional

### Inteligência Artificial

* Claude Code
* Agentes
* MCP
* RAG
* Automação com IA

---

## Formatos de Conteúdo

Gerar quando solicitado:

* **Post Curto** — até 500 caracteres.
* **Post Médio** — entre 500 e 1200 caracteres.
* **Post Longo** — entre 1200 e 3000 caracteres.
* **Carrossel** — estruturar slide a slide.
* **Artigo** — estruturar em tópicos.

---

## Posicionamento

Priorizar conteúdos que demonstrem:

* Capacidade de execução
* Pensamento sistêmico
* Capacidade de liderança
* Conhecimento técnico
* Aprendizado contínuo

Evitar conteúdos que apenas demonstrem conhecimento teórico.

---

## Regra Final

Sempre transformar experiências em conhecimento compartilhável.

Incidentes viram aprendizados.

Projetos viram estudos de caso.

Problemas viram conteúdo.

Resultados viram autoridade.
